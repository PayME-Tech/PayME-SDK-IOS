...
extension UIViewController {
  ...
  func presentModal(
    _ viewControllerToPresent: UIViewController, animated flag: Bool = true,
    completion: (() -> Void)? = nil
  ) {
...
extension UITextField {
  func addDoneButtonOnKeyboard() {
    ...
    let done: UIBarButtonItem = UIBarButtonItem(
      title: "done".localize(), style: .done, target: self, action: #selector(doneButtonAction))
  ...
  func showSpinnerAnimation(onView: UIView) {
    ...
    let animation = LottieAnimation.named("Loading_final", bundle: resourceBundle!)
    ...
    animationView.heightAnchor.constraint(equalToConstant: spinnerView.bounds.width / 4).isActive =
      true
    animationView.play()
...
extension String {
  ...
  func replaceAll(target: String, withString: String) -> String {
    let regex = try! NSRegularExpression(
      pattern: target, options: NSRegularExpression.Options.caseInsensitive)
    let range = NSMakeRange(0, self.count)
    let modString = regex.stringByReplacingMatches(
      in: self, options: [], range: range, withTemplate: withString)
    return modString
  }
...
extension UIView {
  ...
  func createDashedLine(
    from point1: CGPoint, to point2: CGPoint, color: UIColor, strokeLength: NSNumber,
    gapLength: NSNumber, width: CGFloat
  ) {
  ...
  func addLineDashedStroke(
    pattern: [NSNumber]?, radius: CGFloat, color: CGColor, width: CGFloat = 0.5
  ) -> CALayer {
...
extension UIImage {
  ...
  func cropToRect(rect: CGRect!) -> UIImage? {
    ...
    let scaledRect = CGRect(
      x: rect.origin.x * self.scale, y: rect.origin.y * self.scale,
      width: rect.size.width * self.scale, height: rect.size.height * self.scale)
...
extension UIImage {
  ...
  func resizeWithWidth(width: CGFloat) -> UIImage? {
    let imageView = UIImageView(
      frame: CGRect(
        origin: .zero,
        size: CGSize(width: width, height: CGFloat(ceil(width / size.width * size.height)))))
