...
//
  ...
  extension PanModalPresentationController {
     ...
     */
    fileprivate func layoutPresentedView(in containerView: UIView) {
    ...
    fileprivate func adjustPresentedViewFrame() {
      ...
      let adjustedSize = CGSize(
        width: frame.size.width, height: frame.size.height - anchoredYPosition)
     ...
     */
    fileprivate func configureViewLayout() {
  ...
  extension PanModalPresentationController {
    ...
    fileprivate func snap(toYPosition yPos: CGFloat) {
     ...
     */
    fileprivate func adjust(toYPosition yPos: CGFloat) {
      ...
      backgroundView.dimState = .percent(
        1.0 - (yDisplacementFromShortForm / presentedView.frame.height))
    }
  ...
  extension PanModalPresentationController {
    ...
    fileprivate func drawAroundDragIndicator(
      ...
      path.addLine(
        to: CGPoint(
          x: path.currentPoint.x + Constants.dragIndicatorSize.width, y: path.currentPoint.y))
      path.addLine(
        to: CGPoint(x: path.currentPoint.x, y: path.currentPoint.y + totalIndicatorOffset))
    }
  }
