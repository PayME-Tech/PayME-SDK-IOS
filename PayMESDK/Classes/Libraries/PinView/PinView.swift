...
extension KAPinFieldDelegate {
  public func pinField(_ field: KAPinField, didChangeTo string: String, isValid: Bool) {}
}
...
public struct KAPinFieldProperties {
  ...
  public var numberOfCharacters: Int = 6 {
    didSet {
      precondition(
        numberOfCharacters >= 0, "🚫 Number of character must be >= 0, with 0 meaning dynamic")
    }
  }
  public var validCharacters: String = "0123456789" {
    didSet {
      ...
      precondition(
        !validCharacters.contains(token), "🚫 Valid characters can't contain token \"\(token)\"")
    }
  }
  public var token: Character = "•" {
    didSet {
      precondition(
        !validCharacters.contains(token),
        "🚫 token can't be one of the valid characters \"\(token)\"")
...
public class KAPinField: UITextField, UITextFieldDelegate {
  ...
  override public func layoutSubviews() {
    ...
    let textFrame = nsText.boundingRect(
  ...
  private func reload() {
      ...
      if self.inputAccessoryView == nil {
        ...
        numberToolbar.items = [
          UIBarButtonItem.init(
            barButtonSystemItem: .cancel, target: self, action: #selector(cancelNumberPad)),
          UIBarButtonItem(barButtonSystemItem: .flexibleSpace, target: nil, action: nil),
          UIBarButtonItem.init(
            barButtonSystemItem: .done, target: self, action: #selector(doneWithNumberPad)),
        ]
    ...
    self.invisibleField.addTarget(
      self, action: #selector(reloadAppearanceBegin), for: .editingDidBegin)
    ...
    if self.properties.animateFocus {
      if self.timer == nil {
        self.timer = Timer.scheduledTimer(
          timeInterval: 0.6, target: self, selector: #selector(animateFocusedToken), userInfo: nil,
          repeats: true)
        self.timer?.fire()
      }
    } else {
  ...
  @objc private func animateFocusedToken() {
      ...
      let range = self.currentFocusRange
    {
      ...
      if isClear {
        atts[.foregroundColor] =
          self.appearance.tokenFocusColor
      ...
      UIView.transition(
        with: self, duration: duration, options: [.transitionCrossDissolve, .allowUserInteraction],
        animations: {
  ...
  @objc public func reloadAppearance() {
    ...
    let font =
      self.appearance.font?.font() ?? self.font ?? UIFont.preferredFont(forTextStyle: .headline)
    self.attributes = [
    ...
    if isDynamicLength {
      ...
      self.attributedText = NSAttributedString(
        string: self.invisibleField.text!, attributes: attributes)
      return
    }
  ...
  // custom cuc chat =))
  @objc public func reloadAppearanceBegin() {
    guard !self.isAnimating else {
      return
    }
    // Styling backviews
    for v in self.backViews {
      v.alpha = 1.0
      v.backgroundColor = self.appearance.backActiveColor
  ...
  private func updateCursorPosition() {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
      if let position = self.invisibleField.position(
        from: self.invisibleField.beginningOfDocument, offset: offset)
      {
          ...
          range.location >= -1 && range.location < self.properties.numberOfCharacters
        {
          ...
          if sub == String(self.properties.token) {
            ...
            var backIndex =
              self.isRightToLeft ? self.properties.numberOfCharacters - offset - 1 : offset
            backIndex = min(backIndex, self.properties.numberOfCharacters - 1)
            backIndex = max(backIndex, 0)
            if !self.backViews.isEmpty && backIndex < self.backViews.count {
              let backView = self.backViews[backIndex]
              backView.backgroundColor =
                self.appearance.backActiveColor ?? self.appearance.backColor
              backView.layer.borderColor =
                self.appearance.backBorderFocusColor?.cgColor
                ?? self.appearance.backBorderColor.cgColor
            }
          }
        }
      }
