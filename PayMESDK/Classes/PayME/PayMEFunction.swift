...
class PayMEFunction {
  ...
  func checkCondition(_ onError: @escaping ([String: AnyObject]) -> Void) -> Bool {
    if loggedIn == false || dataInit == nil {
      onError([
        "code": PayME.ResponseCode.ACCOUNT_NOT_LOGIN as AnyObject,
        "message": "needLogin".localize() as AnyObject,
      ])
      return false
    }
    if !NetworkReachability.isConnectedToNetwork() {
      onError([
        ...
        "message": "Vui lòng kiểm tra lại đường truyền mạng" as AnyObject,
      ])
      return false
    }
    if !isAccountActivated {
      onError([
        "code": PayME.ResponseCode.ACCOUNT_NOT_ACTIVATED as AnyObject,
        "message": "notActivated".localize() as AnyObject,
      ])
      return false
    }
    if kycState != "APPROVED" {
      onError([
        "code": PayME.ResponseCode.ACCOUNT_NOT_KYC as AnyObject,
        "message": "notKYC".localize() as AnyObject,
      ])
      return false
    }
    return true
  ...
  func checkPayCondition(_ onError: @escaping ([String: AnyObject]) -> Void) -> Bool {
    if loggedIn == false || dataInit == nil {
      onError([
        "code": PayME.ResponseCode.ACCOUNT_NOT_LOGIN as AnyObject,
        "message": "needLogin".localize() as AnyObject,
  ...
  func encryptAES(_ data: String) -> String {
    let aes = try? AES(
      key: Array(webKey.utf8), blockMode: CBC(iv: [UInt8](repeating: 0, count: 16)), padding: .pkcs5
    )
  ...
  func openWallet(
    ...
    if condition {
      ...
      let webViewController = WebViewController(
        payMEFunction: self, nibName: "WebView", bundle: nil)
      ...
      if #available(iOS 11.0, *) {
        if currentVC.view.safeAreaInsets.top > 60 {
          topSafeArea =
            PayME.isRecreateNavigationController ? 1 : currentVC.view.safeAreaInsets.top - 44
        } else {
  ...
  func payAction(
    ...
    if checkPayCondition(onError) {
      if appEnv.isEqual("SANDBOX"), payCode != PayCode.PAYME.rawValue {
        onError([
          "code": PayME.ResponseCode.LIMIT as AnyObject,
          "message": "onlyProduction".localize() as AnyObject,
        ])
        return
      }
      PayME.currentVC = currentVC
      ...
      if amount < PaymentModalController.minAmount {
        onError([
          "code": PayME.ResponseCode.LIMIT as AnyObject,
          "message":
            "Vui lòng thanh toán số tiền lớn hơn \(formatMoney(input: PaymentModalController.minAmount))"
            as AnyObject,
        ])
        return
      }
      if amount > PaymentModalController.maxAmount {
        onError([
          "code": PayME.ResponseCode.LIMIT as AnyObject,
          "message":
            "Vui lòng thanh toán số tiền nhỏ hơn \(formatMoney(input: PaymentModalController.maxAmount))"
            as AnyObject,
        ])
        return
      }
      ...
      request.getMerchantInformation(
        ...
        onSuccess: { response in
          ...
          if data["OpenEWallet"]["GetInfoMerchant"]["succeeded"].boolValue == true {
            curStoreName =
              data["OpenEWallet"]["GetInfoMerchant"]["storeName"].string ?? self.storeName
            curStoreImage =
              data["OpenEWallet"]["GetInfoMerchant"]["storeImage"].string ?? self.storeImage
            isShowHeader = data["OpenEWallet"]["GetInfoMerchant"]["isVisibleHeader"].boolValue
          }

          let orderTransaction = OrderTransaction(
            amount: amount, storeId: storeId, storeName: curStoreName, storeImage: curStoreImage,
            orderId: orderId, note: note ?? "", extraData: extraData ?? "", total: amount,
            isShowHeader: isShowHeader, userName: userName)
  ...
  func getRemainingQuota(
    _ onSuccess: @escaping (Int) -> Void, _ onError: @escaping ([String: AnyObject]) -> Void
  ) {
    request.getTransferMethods(
      payCode: "",
      onSuccess: { response in
        guard
          let quota = (response["Utility"]!["GetPaymentMethod"] as! [String: AnyObject])[
            "remainingQuota"] as? Int
        else {
          return
        }
        onSuccess(quota)
  ...
  func openQRCode(
    ...
    if checkPayCondition(onError) {
      let qrScan = QRScannerController()
        qrScan.setScanSuccess(onScanSuccess: {
            response in
            ...
            _ = detect["message"] as? String ?? "hasError".localize()
            ...
            if succeeded == true {
              ...
              if typename == "DefaultQR" {
                ...
                self.payAction(
                  currentVC: currentViewController, storeId: storeId, userName: userName,
                ...
                if let jsonData = try? JSONSerialization.data(
                  withJSONObject: extraData, options: []),
                  let jsonString = String(data: jsonData, encoding: .utf8)
                {
                  let formattedExtraData = jsonString.replacingOccurrences(of: "\\", with: "\\\\")
                    .replacingOccurrences(of: "\"", with: "\\\"")
                  self.openWallet(
                    false, currentVC, PayME.Action.TRANSFER, amount, note, formattedExtraData, "",
                    false, onSuccessPay, onErrorPay)
      ...
      onSuccess: { response in
        ...
        if succeeded == true {
          if typename == "DefaultQR" {
            ...
            self.payAction(
              currentVC: currentVC, storeId: storeId, userName: userName, orderId: orderId,
              ...
              let jsonString = String(data: jsonData, encoding: .utf8)
            {
              ...
              self.openWallet(
                false, currentVC, PayME.Action.TRANSFER, amount, note, formattedExtraData, "",
                !isShowResultUI, onSuccess, onError)
            } else {
              print("Không thể chuyển đổi extraData thành chuỗi JSON")
            }
          } else {
      ...
      onSuccess: { responseAccessToken in
        ...
        if !(succeeded ?? false), phone == nil || handShake == nil {
          if handShake != nil, phone == nil {
            onError([
              "code": PayME.ResponseCode.ACCOUNT_ERROR as AnyObject,
              "message": "pleaseEnterPhoneNumber".localize() as AnyObject,
            ])
          } else {
            onError([
              "code": PayME.ResponseCode.ACCOUNT_ERROR as AnyObject,
              "message": result["message"] as AnyObject,
            ])
          }
          return
        }
        ...
        self.request.getSetting(
          onSuccess: { success in
            ...
            if let configLimitPayment = configs.first(where: { config in
              ...
              PaymentModalController.maxAmount =
                (configLimitPayment["value"]!["max"] as? Int) ?? 100_000_000
            } else {
              onError([
                "code": PayME.ResponseCode.SYSTEM as AnyObject,
                "message": "cantAccessPaymentConfig".localize() as AnyObject,
              ])
            }
            ...
            if let configService = configs.first(where: { config in
              ...
              if valueConfigService is String {
                values =
                  convertStringToDictionary(text: valueConfigService as! String)!["listService"]
                  as! [AnyObject]
              } else {
              ...
              for value in values {
                ...
                if enable {
                  self.configService.append(
                    ServiceConfig(value["code"] as! String, value["description"] as! String))
                }
              }
            } else {
              onError([
                "code": PayME.ResponseCode.SYSTEM as AnyObject,
                "message": "cantAccessServiceConfig".localize() as AnyObject,
              ])
            }
            ...
            if let configKYCMode = configs.first(where: { config in
              ...
              onError([
                "code": PayME.ResponseCode.SYSTEM as AnyObject,
                "message": "Không lấy được config dịch vụ, vui lòng thử lại sau" as AnyObject,
              ])
            }
            ...
            if let scanModuleConfig = configs.first(where: { config in
              ...
              self.showScanModule =
                enableAppIds?.first(where: { appId in
    ...
    getAccountInfo(
      { response in
        ...
        if self.kycState == "APPROVED" {
          onError([
            "code": PayME.ResponseCode.SYSTEM as AnyObject,
            "message": "kycApproved".localize() as AnyObject,
          ])
          return
        }
        if self.kycState == "PENDING" {
          self.openWallet(
            false, currentVC, PayME.Action.OPEN, nil, "", "", "", false, { _ in }, onError)
          return
        }
        if self.kycMode == nil {
          onError([
            "code": PayME.ResponseCode.SYSTEM as AnyObject,
            "message": "Không lấy được config KYC, vui lòng thử lại sau" as AnyObject,
          ])
          return
        }
        ...
        let kycController = KYCController(
          payMEFunction: self, flowKYC: self.kycMode!, onSuccess: onSuccess)
        kycController.kyc()
      },
      { error in
        onError(error)
    ...
    initSDK(
      onSuccess: { _ in
        ...
        if !self.isAccountActivated {
          onSuccess([
            "code": PayME.KYCState.NOT_ACTIVATED as AnyObject,
            "message": "notActivated".localize() as AnyObject,
          ])
          return
        }
        ...
        default:
          onSuccess([
            "code": PayME.KYCState.NOT_KYC as AnyObject,
            "message": "notKYC".localize() as AnyObject,
          ])
          return
        }
        if self.accessToken != "", self.kycState == "APPROVED" {
          onSuccess([
            "code": PayME.KYCState.KYC_APPROVED as AnyObject,
            "message": "loginSuccess".localize() as AnyObject,
          ])
          return
        }
      },
  ...
  func getService(
    ...
    request.getService(
      onSuccess: { success in onSuccess(success) }, onError: { error in onError(error) })
  }

  func getSupportedServices(
    _ onSuccess: @escaping ([ServiceConfig]) -> Void,
    _ onError: @escaping ([String: AnyObject]) -> Void
  ) {
    if loggedIn == false || dataInit == nil {
      onError([
