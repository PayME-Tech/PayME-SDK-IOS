...
class PaymentPresentation {
  ...
  init(
    ...
    onPaymeError: @escaping (String) -> Void = { _ in
    }
  ) {
    ...
    self.onError = { dictionary in
      ...
      if code == PayME.ResponseCode.SYSTEM {
        paymentViewModel.paymentSubject.onNext(
          PaymentState(
            state: .ERROR,
            error: ResponseError(
              code: .SERVER_ERROR,
              message: dictionary["message"] as? String ?? "hasError".localize())))
      } else {
        onError(dictionary)
      }
    }
  ...
  func paymentPayMEMethod(securityCode: String, orderTransaction: OrderTransaction) {
    request.transferWallet(
      storeId: orderTransaction.storeId, userName: orderTransaction.userName,
      orderId: orderTransaction.orderId, securityCode: securityCode,
      extraData: orderTransaction.extraData, note: orderTransaction.note,
        ...
        if succeeded == true {
          ...
          let result = Result(
            ...
            transactionInfo: TransactionInformation(
              transaction: transactionNumber, transactionTime: formatDate),
            extraData: responseSuccess
          )
          self.paymentViewModel.paymentSubject.onNext(
            PaymentState(state: State.RESULT, result: result))
        } else {
          let result = Result(
      ...
      onError: { error in
        if let code = error["code"] as? Int {
          if code == 401 {
            self.paymentViewModel.paymentSubject.onNext(
              PaymentState(
                state: State.ERROR, error: ResponseError(code: ResponseErrorCode.EXPIRED)))
          }
        }
        self.onError(error)
      },
  ...
  func paymentCreditWallet(securityCode: String, orderTransaction: OrderTransaction) {
    request.creditWallet(
      storeId: orderTransaction.storeId, orderId: orderTransaction.orderId,
  ...
  func transferByLinkedBank(
    transaction: String, orderTransaction: OrderTransaction, linkedId: Int, OTP: String
  ) {
    request.transferByLinkedBank(
      transaction: transaction, storeId: orderTransaction.storeId,
      userName: orderTransaction.userName, orderId: orderTransaction.orderId, linkedId: linkedId,
      extraData: orderTransaction.extraData, note: orderTransaction.note, otp: OTP,
      amount: orderTransaction.amount,
      onSuccess: { response in
        let paymentInfo = response["OpenEWallet"]!["Payment"] as! [String: AnyObject]
        if let payInfo = paymentInfo["Pay"] as? [String: AnyObject] {
              ...
              if state == "INVALID_OTP" {
                self.paymentViewModel.paymentSubject.onNext(
                  PaymentState(
                    state: State.ERROR,
                    error: ResponseError(
                      code: ResponseErrorCode.INVALID_OTP,
                      message: payInfo["message"] as? String ?? "hasError".localize()
                    )))
              }
              if state == "FAILED" {
                let message = payInfo["message"] as? String
                let result = Result(
                  ...
                  extraData: [
                    "code": PayME.ResponseCode.PAYMENT_ERROR as AnyObject,
                    "message": (message ?? "hasError".localize()) as AnyObject,
                  ]
                )
                self.paymentViewModel.paymentSubject.onNext(
                  PaymentState(state: State.RESULT, result: result))
          ...
          self.onError([
            ...
            "message": "hasError".localize() as AnyObject,
          ])
        }
      },
      onError: { error in
  ...
  func authenCreditCard(orderTransaction: OrderTransaction) {
    ...
    request.authenCreditCard(
      cardNumber: cardNum, expiredAt: exp, linkedId: linkedId,
      onSuccess: { response in
          ...
          let isAuth = data["CreditCardLink"]["AuthCreditCard"]["isAuth"].bool
        {
          if isSucceeded == true {
            if isAuth == true {
              ...
              let realHtml =
                "<html><body onload=\"document.forms[0].submit();\">\(html)</body></html>"
              self.paymentViewModel.paymentSubject.onNext(
                PaymentState(
                  state: State.ERROR, orderTransaction: orderTransaction,
            ...
            let result = Result(
              type: ResultType.FAIL,
              failReasonLabel: data["CreditCardLink"]["AuthCreditCard"]["message"].string
                ?? "hasError".localize(),
              orderTransaction: orderTransaction,
              transactionInfo: TransactionInformation(
                transaction: orderTransaction.orderId, transactionTime: toDateString(date: Date())),
              extraData: [
                "code": PayME.ResponseCode.PAYMENT_ERROR as AnyObject,
                "message":
                  (data["CreditCardLink"]["AuthCreditCard"]["message"].string
                  ?? "hasError".localize()) as AnyObject,
              ]
            )
            self.paymentViewModel.paymentSubject.onNext(
              PaymentState(state: State.RESULT, result: result))
          ...
          let result = Result(
            ...
            extraData: [
              "code": PayME.ResponseCode.PAYMENT_ERROR as AnyObject,
              "message":
                (data["CreditCardLink"]["AuthCreditCard"]["message"].string ?? "hasError".localize())
                as AnyObject,
            ]
          )
          self.paymentViewModel.paymentSubject.onNext(
              ...
              if state == "REQUIRED_OTP" {
                ...
                self.paymentViewModel.paymentSubject.onNext(
                  PaymentState(
                    state: State.ERROR,
                    error: ResponseError(
                      code: ResponseErrorCode.REQUIRED_OTP, transaction: transaction))
                )
              } else if state == "REQUIRED_VERIFY" {
                if let html = payment["html"] as? String {
                  let realHtml =
                    (orderTransaction.paymentMethod?.dataLinked?.swiftCode != nil
                      ? html
                      : "<html><body onload=\"document.forms[0].submit();\">\(html)</body></html>")
                  self.paymentViewModel.paymentSubject.onNext(
                    PaymentState(
  ...
  func payBankTransfer(orderTransaction: OrderTransaction) {
    paymentViewModel.paymentSubject.onNext(
      PaymentState(
        state: .BANK_TRANS_RESULT, orderTransaction: orderTransaction, bankTransferState: .PENDING))
    request.paymentBankTransfer(
      storeId: orderTransaction.storeId, userName: orderTransaction.userName,
      orderId: orderTransaction.orderId, extraData: orderTransaction.extraData,
          ...
          if let payment = payInfo["payment"] as? [String: AnyObject] {
            ...
            if let bankTranferState = payment["bankTranferState"] as? String {
              if bankTranferState == "SUCCEEDED" {
                ...
                self.paymentViewModel.paymentSubject.onNext(
                  PaymentState(
                    state: .BANK_TRANS_RESULT, orderTransaction: orderTransaction,
                    bankTransferState: .FAIL))
              } else {
                let message = payment["message"] as? String
                let result = Result(
                  type: ResultType.FAIL,
  ...
  func payVNQRCode(orderTransaction: OrderTransaction, redirectURL: String) -> URLSessionDataTask? {
    request.payVNPayQRCode(
      storeId: orderTransaction.storeId, userName: orderTransaction.userName,
      orderId: orderTransaction.orderId, extraData: orderTransaction.extraData,
      note: orderTransaction.note, amount: orderTransaction.amount, redirectUrl: redirectURL,
      failedUrl: "",
      onSuccess: { success in
        let data = JSON(success)
        if let qrContent = data["OpenEWallet"]["Payment"]["Pay"]["payment"]["qrContent"].string,
          let transaction = data["OpenEWallet"]["Payment"]["Pay"]["history"]["payment"][
            "transaction"
          ].string
        {
          ...
          let result = Result(
            type: ResultType.FAIL,
            failReasonLabel: data["OpenEWallet"]["Payment"]["Pay"]["message"].string
              ?? "hasError".localize(),
            orderTransaction: orderTransaction,
            transactionInfo: TransactionInformation(
              transaction: orderTransaction.orderId, transactionTime: toDateString(date: Date())),
            extraData: [
              "code": PayME.ResponseCode.PAYMENT_ERROR as AnyObject,
              "message":
                (data["OpenEWallet"]["Payment"]["Pay"]["message"].string ?? "hasError".localize())
                as AnyObject,
            ]
          )
          self.paymentViewModel.paymentSubject.onNext(
  ...
  func payCreditCard(orderTransaction: OrderTransaction) {
    request.transferCreditCard(
      storeId: orderTransaction.storeId, userName: orderTransaction.userName,
      orderId: orderTransaction.orderId, extraData: orderTransaction.extraData,
      note: orderTransaction.note,
      cardNumber: orderTransaction.paymentMethod!.dataCreditCard!.cardNumber,
      cardHolder: orderTransaction.paymentMethod!.dataCreditCard!.cardHolder,
      expiredAt: orderTransaction.paymentMethod!.dataCreditCard!.expiredAt,
      cvv: orderTransaction.paymentMethod!.dataCreditCard!.cvv,
      refId: orderTransaction.paymentMethod!.dataCreditCard?.referenceId ?? "",
      amount: orderTransaction.amount,
      onSuccess: { success in
        let payment = success["OpenEWallet"]!["Payment"] as! [String: AnyObject]
        if let payInfo = payment["Pay"] as? [String: AnyObject] {
  ...
  func getListMethods(
    ...
    request.getTransferMethods(
      payCode: payCode,
      onSuccess: { response in
        let items =
          (response["Utility"]!["GetPaymentMethod"] as! [String: AnyObject])["methods"]
          as! [[String: AnyObject]]
        ...
        for (_, item) in items.enumerated() {
          ...
          let methodInformation = PaymentMethod(
            type: methodType, title: item["title"] as? String ?? "",
            label: item["label"] as? String ?? "", iconUrl: item["iconUrl"] as? String ?? ""
          )
          if methodType == "WALLET" {
          ...
          if methodType == "CREDIT_BALANCE" {
            methodInformation.dataCreditWallet = CreditWalletInformation(
              supplierLinkedId: (item["data"] as! [String: AnyObject])["supplierLinkedId"]
                as? String
            )
          }
  ...
  func getLinkBank(orderTransaction: OrderTransaction) {
    request.getBankList(
      onSuccess: { bankListResponse in
        let banks = bankListResponse["Setting"]!["banks"] as! [[String: AnyObject]]
        var listBank: [Bank] = []
        for bank in banks {
          if bank["depositable"] as? Bool ?? false, (bank["cardNumberLength"] as? Int) != nil,
            (bank["cardPrefix"] as? String) != nil
          {
            ...
            let temp = Bank(
              id: bank["id"] as! Int, cardNumberLength: bank["cardNumberLength"] as! Int,
              cardPrefix: bank["cardPrefix"] as! String, enName: bank["enName"] as! String,
              viName: bank["viName"] as! String,
        ...
        if orderTransaction.paymentMethod?.type == MethodType.BANK_TRANSFER.rawValue {
          ...
          self.paymentViewModel.paymentSubject.onNext(
            PaymentState(state: State.ATM, banks: listBank, orderTransaction: orderTransaction))
        }
      },
  ...
  func createVietQR(orderTransaction: OrderTransaction) {
    request.createVietQR(
      storeId: orderTransaction.storeId, userName: orderTransaction.userName,
      orderId: orderTransaction.orderId, note: orderTransaction.note,
      amount: orderTransaction.amount,
      onSuccess: { response in
        let paymentInfo = response["OpenEWallet"]!["Payment"] as! [String: AnyObject]
        ...
        let formatDate =
          history["createdAt"].string != nil
          ? toDateString(date: toDate(dateString: history["createdAt"].stringValue)) : ""
        let transactionNumber = history["payment"]["transaction"].stringValue
        let payment = payInfo["payment"]
        if payment["state"].stringValue == "REQUIRED_TRANSFER",
          let vietQRCode = payment["qrContent"].string
        {
          let paymeBank = payment["banks"].arrayValue[0]
          let dataVietQR = VietQRInformation(
            qrContent: vietQRCode,
            manualInfo: paymeBank.dictionary != nil
              ? PayMEQRTransfer(
                bankName: paymeBank["bankName"].stringValue,
                shortBankName: paymeBank["bankShortName"].stringValue,
                bankNumber: paymeBank["bankNumber"].stringValue,
                accountName: paymeBank["fullName"].stringValue,
                branchName: paymeBank["branch"].stringValue,
                content: paymeBank["content"].stringValue) : nil)
          orderTransaction.paymentMethod?.dataVietQR = dataVietQR
          self.paymentViewModel.paymentSubject.onNext(
            PaymentState(
              state: .VIET_QR, orderTransaction: orderTransaction,
              error: ResponseError(
                code: .REQUIRED_VERIFY,
                transactionInformation: TransactionInformation(
                  transaction: transactionNumber, transactionTime: formatDate))))
        } else {
  ...
  func getFee(orderTransaction: OrderTransaction) {
    ...
    request.getFee(
      amount: orderTransaction.amount, payment: payment,
      onSuccess: { response in
        let data = JSON(response)
        print(data)
        if data["Utility"]["GetFee"]["succeeded"].boolValue {
          if let state = (response["Utility"]!["GetFee"] as? [String: AnyObject])?["state"]
            as? String
          {
            if state == "OVER_DAY_QUOTA" || state == "OVER_MONTH_QUOTA" {
              self.paymentViewModel.paymentSubject.onNext(
                PaymentState(
                  state: State.ERROR,
                  error: ResponseError(
                    code: ResponseErrorCode.OVER_QUOTA,
                    message: (response["Utility"]!["GetFee"] as? [String: AnyObject])?["message"]
                      as? String ?? "overQuota".localize())))
              return
            }
          }
          orderTransaction.total =
            orderTransaction.amount + data["Utility"]["GetFee"]["fee"]["fee"].intValue
          self.paymentViewModel.paymentSubject.onNext(
            PaymentState(state: State.FEE, orderTransaction: orderTransaction))
        } else {
          self.onError([
            "code": PayME.ResponseCode.PAYMENT_ERROR as AnyObject,
            "message": (data["Utility"]["GetFee"]["message"].string ?? "hasError".localize())
              as AnyObject,
          ])
        ...
        if let transInfo = payment["GetTransactionInfo"] as? [String: AnyObject] {
          ...
          let result: Result? = {
            ...
            if state == "PENDING" {
              if isAcceptPending {
                let responseError =
                  ["code": PayME.ResponseCode.PAYMENT_PENDING as AnyObject] as [String: AnyObject]
                let textPending =
                  "Cần thời gian thêm để xử lý. Vui lòng không thực hiện lại tránh bị trùng. Liên hệ CSKH để hỗ trợ 1900 88 66 65"
                return Result(
                  type: ResultType.PENDING, failReasonLabel: textPending,
                  orderTransaction: orderTransaction, transactionInfo: transactionInfo,
                  extraData: responseError)
              } else {
                return nil
              }
              ...
              return Result(
                type: ResultType.SUCCESS, orderTransaction: orderTransaction,
                transactionInfo: transactionInfo, extraData: responseSuccess)
            } else {
              let responseError =
                [
                  "code": PayME.ResponseCode.PAYMENT_ERROR as AnyObject,
                  "message": (transInfo["reason"] as? String ?? "hasError".localize()) as AnyObject,
                ] as [String: AnyObject]
              return Result(
                type: ResultType.FAIL, failReasonLabel: transInfo["reason"] as? String ?? "",
                orderTransaction: orderTransaction, transactionInfo: transactionInfo,
                extraData: responseError)
            }
          }()
  ...
  func getListBankManual(orderTransaction: OrderTransaction, listSettingBank: [Bank]) {
    request.getListBankManual(
      storeId: orderTransaction.storeId, userName: orderTransaction.userName,
      orderId: orderTransaction.orderId, amount: orderTransaction.amount,
      onSuccess: { response in
        guard
          let payInfo = (response["OpenEWallet"]!["Payment"] as? [String: AnyObject])?["Pay"]
            as? [String: AnyObject]
        else {
          self.onError([
            "code": PayME.ResponseCode.SYSTEM as AnyObject,
        ...
        guard let isSucceed = payInfo["succeeded"] as? Bool else {
          self.onError([
            "code": PayME.ResponseCode.PAYMENT_ERROR as AnyObject,
            "message": (payInfo["message"] as? String ?? "hasError".localize()) as AnyObject,
          ])
          return
        }
        if isSucceed == false {
        ...
        if bankList.count > 0 {
          ...
          self.paymentViewModel.paymentSubject.onNext(
            PaymentState(
              state: State.BANK_TRANSFER, banks: listSettingBank, listBankManual: listBank,
              orderTransaction: orderTransaction))
        } else {
          self.onPaymeError("")
          PaymentModalController.isShowCloseModal = false
          PayME.currentVC!.dismiss(animated: true) {
            self.onError([
              "code": PayME.ResponseCode.PAYMENT_ERROR as AnyObject,
              "message": "manualBankNotFound".localize() as AnyObject,
            ])
          }
        }
      },
