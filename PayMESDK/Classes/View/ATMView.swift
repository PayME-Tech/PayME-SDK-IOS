...
class ATMView: UIView {
  ...
  func updateUIByMethod(orderTransaction: OrderTransaction) {
    ...
    if delegate != nil {
      if delegate!.isShowScan() == true {
        ...
        cardInput.updateExtraIcon(
          iconImage: imageSVG?.uiImage,
          onPress: {
    ...
    case MethodType.LINKED.rawValue:
      if method.dataLinked != nil {
        let url = URL(
          string:
            "https://static.payme.vn/image_bank/image_method/method\(method.dataLinked!.swiftCode ?? method.dataLinked!.issuer)@2x.png"
        )
  ...
  let cardInput = InputView(
    title: "fillCardNumberUpper".localize(), placeholder: "inputCardNumber".localize(),
    keyboardType: .numberPad)
  let nameInput = InputView(
    title: "fillFullNameCardHolderUpper".localize(), placeholder: "fullnameCardHolder".localize(),
    isAutoCapitalization: true)
  let dateInput = InputView(
    title: "releaseDate".localize().uppercased(), placeholder: "MM/YY", keyboardType: .numberPad)
  let cvvInput = InputView(
    title: "cvvUppercase".localize(), placeholder: "CVV/CVC", keyboardType: .numberPad)
