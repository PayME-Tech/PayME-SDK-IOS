...
class BankTransferView: UIView {
  ...
  var onPressOpenVietQRBanks: () -> Void = {}
  ...
  func setupUI() {
    ...
    titleLabel.leadingAnchor.constraint(equalTo: bankContainer.leadingAnchor, constant: 16)
      .isActive = true

    contentLabel.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: 8).isActive = true
    contentLabel.leadingAnchor.constraint(equalTo: bankContainer.leadingAnchor, constant: 16)
      .isActive = true
    contentLabel.trailingAnchor.constraint(equalTo: buttonBank.leadingAnchor, constant: -4)
      .isActive = true

    buttonBank.centerYAnchor.constraint(equalTo: bankContainer.centerYAnchor).isActive = true
    buttonBank.trailingAnchor.constraint(equalTo: bankContainer.trailingAnchor, constant: -16)
      .isActive = true
    ...
    bankContainer.bottomAnchor.constraint(equalTo: contentLabel.bottomAnchor, constant: 8)
      .isActive = true

    hStack.topAnchor.constraint(equalTo: transferInfo.topAnchor, constant: 4).isActive = true
    hStack.leadingAnchor.constraint(equalTo: transferInfo.leadingAnchor, constant: 16).isActive =
      true
    hStack.trailingAnchor.constraint(equalTo: transferInfo.trailingAnchor, constant: -16).isActive =
      true
  ...
  func updateInfo(bank: BankManual?, orderTransaction: OrderTransaction) {
    ...
    bankLogo.load(
      url: "https://static.payme.vn/image_bank/icon_banks/icon\(paymeBank.swiftCode)@2x.png")
    ...
    let normalText1 = NSMutableAttributedString(
      string: "bankTransferContent1".localize(),
      attributes: [
        .font: UIFont.systemFont(ofSize: 14, weight: .regular),
        .foregroundColor: UIColor(0, 0, 0),
      ])
    let normalText2 = NSMutableAttributedString(
      string: "bankTransferContent2".localize(),
      attributes: [
        .font: UIFont.systemFont(ofSize: 14, weight: .regular),
        .foregroundColor: UIColor(0, 0, 0),
      ])
    normalText1.append(
      NSMutableAttributedString(
        string: "\(formatMoney(input: orderTransaction.total)) đ ",
        attributes: [
    ...
    if paymeBank.qrCode != "" {
      ...
      layoutIfNeeded()
      qrView?.addLineDashedStroke(
        pattern: [2, 2], radius: 16, color: UIColor(142, 142, 142).cgColor)
    }
    updateConstraints()
    layoutIfNeeded()
    ...
    transferInfo.addLineDashedStroke(
      pattern: [2, 2], radius: 16, color: UIColor(142, 142, 142).cgColor)
  }
  ...
  let openVietQRBanksButton: UIButton = {
    ...
    button.setAttributedTitle(
      NSAttributedString(
        string: "openVietQRBankList".localize() + " ",
        attributes: [
          .font: UIFont.systemFont(ofSize: 14, weight: .regular),
