...
class InformationRow: UIStackView {
  ...
  init(
    key: String, value: String, color: UIColor? = UIColor(4, 4, 4),
    font: UIFont? = .systemFont(ofSize: 15, weight: .medium),
    keyColor: UIColor? = UIColor(100, 112, 129),
    keyFont: UIFont? = .systemFont(ofSize: 15, weight: .light), allowCopy: Bool = false
  ) {
    self.key = key
    self.value = value
  ...
  func setUpUI() {
    ...
    if allowCopy == true {
      ...
      let singleTap: UITapGestureRecognizer = UITapGestureRecognizer(
        target: self, action: #selector(onPressCopy))
      ...
      for (idx, info) in data.enumerated() {
        ...
        if idx > 0 {
          ...
          seperator.createDashedLine(
            from: CGPoint(x: 0, y: 0), to: CGPoint(x: stackView.frame.size.width, y: 0),
            color: UIColor(203, 203, 203), strokeLength: 4, gapLength: 4, width: 1)
        }
        let row = InformationRow(
          key: key, value: value, color: info["color"] as? UIColor, font: info["font"] as? UIFont,
          keyColor: info["keyColor"] as? UIColor, keyFont: info["keyFont"] as? UIFont,
          allowCopy: info["allowCopy"] as? Bool ?? false)
