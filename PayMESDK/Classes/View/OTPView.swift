...
internal class OTPView: UIView {
  ...
  init() {
    ...
    txtErrorMessage.trailingAnchor.constraint(equalTo: trailingAnchor, constant: -30).isActive =
      true
    txtErrorMessage.leadingAnchor.constraint(equalTo: leadingAnchor, constant: 30).isActive = true

    sendOtpButton.topAnchor.constraint(equalTo: txtErrorMessage.bottomAnchor, constant: 8)
      .isActive = true
  ...
  func startCountDown(from: Int = 0) {
    if countdownValue == 0 {
      countdownValue = from
      sendOtpButton.setTitle(
        "\("resendOTP".localize()) (\(String(format: "%02d", countdownValue / 60)):\(String(format: "%02d", countdownValue % 60)))",
        for: .disabled)
      sendOtpButton.isEnabled = false
      Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { timer in
        if self.countdownValue > 0 {
          self.sendOtpButton.setTitle(
            "\("resendOTP".localize()) (\(String(format: "%02d", self.countdownValue / 60)):\(String(format: "%02d", self.countdownValue % 60)))",
            for: .disabled)
