...
class OrderView: UIView {
  ...
  init(
    amount: Int = 0, storeName: String = "", serviceCode: String = "", note: String = "",
    logoUrl: String? = nil, isFullInfo: Bool = false
  ) {
    ...
    storeNameRow = InformationRow(
      key: "receiveName".localize(), value: storeName, color: .white,
      font: .systemFont(ofSize: 14, weight: .bold),
      keyColor: .white, keyFont: .systemFont(ofSize: 14, weight: .regular))
    serviceRow = InformationRow(
      key: "serviceCode".localize(), value: serviceCode, color: .white,
      font: .systemFont(ofSize: 14, weight: .regular),
      keyColor: .white, keyFont: .systemFont(ofSize: 14, weight: .regular))
    noteRow = InformationRow(
      key: "content".localize(), value: note, color: .white,
      font: .systemFont(ofSize: 14, weight: .regular),
      keyColor: .white, keyFont: .systemFont(ofSize: 14, weight: .regular))
  ...
  override func layoutSubviews() {
    ...
    if vStack.isDescendant(of: self) && seperator.isDescendant(of: vStack) {
      seperator.createDashedLine(
        from: CGPoint(x: 0, y: 0), to: CGPoint(x: vStack.frame.size.width, y: 0),
        color: UIColor(203, 203, 203), strokeLength: 2, gapLength: 2, width: 1)
    }
  }
