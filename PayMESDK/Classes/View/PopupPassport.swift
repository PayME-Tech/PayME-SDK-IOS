...
class PopupPassport: UIView {
  ...
  init() {
    ...
    imageSVG?.fillColor(
      color: UIColor(hexString: PayME.configColor[0]), opacity: 1, defaultColor: "#0AB822")
    ...
    buttonContainer.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: 24).isActive =
      true
    ...
    buttonContainer.widthAnchor.constraint(equalTo: rootView.widthAnchor, constant: -34).isActive =
      true
    buttonContainer.bottomAnchor.constraint(equalTo: rootView.bottomAnchor, constant: -20)
      .isActive = true
  }
