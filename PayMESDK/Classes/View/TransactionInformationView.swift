...
class TransactionInformationView: UIView {
  ...
  func setUpUI() {
    ...
    let vStackId = InformationColumn(
      key: "transactionCode".localize(), value: id ?? "N/A", textAlign: .left)
    let vStackTime = InformationColumn(
      key: "transactionTime".localize(), value: timeStamp ?? "N/A", textAlign: .right)
