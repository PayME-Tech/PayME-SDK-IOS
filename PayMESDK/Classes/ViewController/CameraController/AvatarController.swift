...
class AvatarController: UIViewController, UIImagePickerControllerDelegate,
  ...
  func initializeCaptureSession() {
    ...
    guard
      let camera = AVCaptureDevice.default(
        .builtInWideAngleCamera, for: AVMediaType.video, position: .front)
    else {
    ...
    do {
      if cameraCaptureInput == nil && cameraCaptureOutput == nil {
        ...
        cameraPreviewLayer?.frame = CGRect(
          x: 16, y: 120, width: screenSize.width - 32, height: screenSize.width - 32)
...
extension AvatarController: AVCapturePhotoCaptureDelegate {
  func photoOutput(
        ...
        forJPEGSampleBuffer: sampleBuffer, previewPhotoSampleBuffer: previewPhotoSampleBuffer)
    {
      ...
      let resizeImage = finalImage.resizeImage(
        targetSize: CGSize(width: screenSize.width - 32, height: screenSize.width - 32))
      session.stopRunning()
