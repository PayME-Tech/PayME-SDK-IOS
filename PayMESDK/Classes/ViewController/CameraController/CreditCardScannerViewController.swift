...
//
    ...
    public protocol CreditCardScannerViewControllerDelegate: AnyObject {
      ...
      func creditCardScannerViewController(
        _ viewController: CreditCardScannerViewController, didFinishWith card: CreditCard)
    }

    @available(iOS 13, *)
    extension CreditCardScannerViewControllerDelegate where Self: UIViewController {
      public func creditCardScannerViewControllerDidCancel(
        _ viewController: CreditCardScannerViewController
      ) {
      ...
      override open func viewDidLoad() {
        ...
        AVCaptureDevice.authorize { [weak self] authoriazed in
          ...
          guard authoriazed else {
            strongSelf.delegate?.creditCardScannerViewController(
              strongSelf,
              didErrorWith: CreditCardScannerError(kind: .authorizationDenied, underlyingError: nil)
            )
            return
          }
    ...
    extension CreditCardScannerViewController {
      @objc fileprivate func cancel(_ sender: UIButton) {
        delegate?.creditCardScannerViewControllerDidCancel(self)
      }
      ...
      fileprivate func layoutSubviews() {
        ...
        NSLayoutConstraint.activate([
          ...
          cameraView.heightAnchor.constraint(
            equalTo: cameraView.widthAnchor, multiplier: CreditCard.heightRatioAgainstWidth,
            constant: 100),
        ])
        ...
        bottomStackView.directionalLayoutMargins = .init(
    ...
    extension CreditCardScannerViewController: ImageAnalyzerProtocol {
      internal func didFinishAnalyzation(
        with result: Swift.Result<CreditCard, CreditCardScannerError>
      ) {
        switch result {
        case let .success(creditCard):
          DispatchQueue.main.async { [weak self] in
            ...
            strongSelf.cameraView.stopSession()
            strongSelf.delegate?.creditCardScannerViewController(
              strongSelf, didFinishWith: creditCard)
          }
