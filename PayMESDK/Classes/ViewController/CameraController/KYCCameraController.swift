...
class KYCCameraController: UIViewController, UIImagePickerControllerDelegate,
  UINavigationControllerDelegate
{
  ...
  override func viewDidLoad() {
    ...
    if #available(iOS 11, *) {
      ...
      NSLayoutConstraint.activate([
        ...
        titleLabel.topAnchor.constraint(
          equalToSystemSpacingBelow: guide.topAnchor, multiplier: 1.3),
      ...
      NSLayoutConstraint.activate([
        ...
        titleLabel.topAnchor.constraint(
          equalTo: topLayoutGuide.bottomAnchor, constant: standardSpacing + 5),
        getPhoto.bottomAnchor.constraint(equalTo: bottomLayoutGuide.topAnchor, constant: -40),
        pressCamera.bottomAnchor.constraint(
          equalTo: bottomLayoutGuide.topAnchor, constant: -standardSpacing),
      ])
    }
    ...
    choiceDocumentType.leadingAnchor.constraint(equalTo: self.view.leadingAnchor, constant: 16)
      .isActive = true
    choiceDocumentType.topAnchor.constraint(equalTo: backButton.bottomAnchor, constant: 37)
      .isActive = true
    ...
    guideLabel.topAnchor.constraint(
      equalTo: choiceDocumentType.bottomAnchor,
      constant: (self.cameraPreviewLayer?.bounds.height ?? (screenSize.width - 32) * 0.67) + 60
    ).isActive = true
    ...
    popupPassport.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 30).isActive =
      true
    popupPassport.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -30).isActive =
      true
    ...
    popupPassport.continueButton.addTarget(
      self, action: #selector(acceptOptionPassport), for: .touchUpInside)
    popupPassport.cancelButton.addTarget(
      self, action: #selector(closePopupPassport), for: .touchUpInside)
    ...
    kycDocumentController.setOnSuccessChoiceKYC(onSuccessChoiceKYC: { response in
      DispatchQueue.main.async {
        if response == 0 {
          ...
          self.choiceDocumentType.imageEdgeInsets = UIEdgeInsets(
            top: 0, left: 185, bottom: 0, right: 0)  //adjust these to have fit right
          self.active = response
        }
        if response == 1 {
          self.choiceDocumentType.setTitle("identifyCitizen".localize(), for: .normal)
          self.choiceDocumentType.imageEdgeInsets = UIEdgeInsets(
            top: 0, left: 160, bottom: 0, right: 0)  //adjust these to have fit right
          self.active = response
        }
        if response == 2 {
  ...
  let backButton: UIButton = {
    let button = UIButton()
    button.setImage(
      UIImage(for: KYCCameraController.self, named: "previous")?.resizeWithWidth(width: 16),
      for: .normal)
    button.translatesAutoresizingMaskIntoConstraints = false
    return button
  }()
    ...
    do {
      if cameraCaptureInput == nil && cameraCaptureOutput == nil {
        ...
        cameraPreviewLayer?.frame = CGRect(
          x: 16, y: 160, width: screenSize.width - 32, height: (screenSize.width - 32) * 0.67)
  ...
  @objc func imagePickerController(
    ...
    guard let image = info[UIImagePickerController.InfoKey.originalImage.rawValue] as? UIImage
    else {
      return
    }
  ...
  func photoOutput(
    ...
    if let sampleBuffer = photoSampleBuffer,
      let dataImage = AVCapturePhotoOutput.jpegPhotoDataRepresentation(
        forJPEGSampleBuffer: sampleBuffer, previewPhotoSampleBuffer: previewPhotoSampleBuffer)
    {
      let image: UIImage = UIImage(data: dataImage)!
      let originalSize: CGSize
      let visibleLayerFrame = cameraPreviewLayer!.bounds
      let metaRect: CGRect =
        (cameraPreviewLayer?.metadataOutputRectConverted(fromLayerRect: visibleLayerFrame))!
