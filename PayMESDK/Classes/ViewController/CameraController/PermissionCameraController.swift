...
class PermissionCamera: UIViewController, UINavigationControllerDelegate {
  ...
  let backButton: UIButton = {
    let button = UIButton()
    button.setImage(
      UIImage(for: KYCFrontController.self, named: "icSetArrowBack32Px"), for: .normal)
  ...
  let iconNullCamera: UIImageView = {
    var bgImage = UIImageView(
      image: UIImage(for: QRScannerController.self, named: "iconArtNullNoCameraAccess"))
    bgImage.translatesAutoresizingMaskIntoConstraints = false
    return bgImage
  }()

  public func setOnSuccess(onSuccess: @escaping (String) -> Void) {
  ...
  override func viewDidLoad() {
    ...
    titleLabel.text =
      ...
      ? "permissionContent".localize() : "permissionMemory".localize()
    contentLabel.text =
      (type == PermissionType.CAMERA)
      ? "permissionContent1".localize() : "permissionMemoryDescription".localize()
