...
class VisionModules: CreditCardScannerViewControllerDelegate {
  ...
  func creditCardScannerViewController(
    _ viewController: CreditCardScannerViewController, didErrorWith error: CreditCardScannerError
  ) {
    vc.dismiss(animated: true) {
  ...
  func creditCardScannerViewController(
    ...
    vc.dismiss(animated: true) {
      self.onSuccess([
        ...
        "cardExpiredDate": card.expireDate != nil
          ? "\(self.getMonth(month: card.expireDate?.month))/\(self.getYear(year: card.expireDate?.year))"
          : "",
        "cardValidDate": card.validDate != nil
          ? "\(self.getMonth(month: card.validDate?.month))/\(self.getYear(year: card.validDate?.year))"
          : "",
      ])
    }
  }
