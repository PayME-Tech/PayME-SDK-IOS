...
class Method: UITableViewCell {
  ...
  func configure(
    with presentable: PaymentMethod, payMEFunction: PayMEFunction,
    orderTransaction: OrderTransaction
  ) {
    if presentable.type == MethodType.WALLET.rawValue {
      ...
      if payMEFunction.accessToken == "" {
        ...
        methodView.onPress = {
          PayME.currentVC?.dismiss(animated: true) {
            payMEFunction.KYC(PayME.currentVC!, {}, { dictionary in })
          }
        }
      } else {
      ...
      case MethodType.LINKED.rawValue:
        if presentable.dataLinked != nil {
          let url = URL(
            string:
              "https://static.payme.vn/image_bank/image_method/method\(presentable.dataLinked!.swiftCode ?? presentable.dataLinked!.issuer)@2x.png"
          )
