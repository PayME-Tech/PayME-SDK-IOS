...
class WebViewController: UIViewController, WKUIDelegate, WKScriptMessageHandler,
  WKNavigationDelegate, PanModalPresentable
{
  ...
  let reachability = Alamofire.NetworkReachabilityManager(host: "www.google.com")
  private var onSuccessWebView: ((String) -> Void)? = nil
  private var onFailWebView: ((String) -> Void)? = nil
  private var onSuccess: (([String: AnyObject]) -> Void)? = nil
  private var onError: (([String: AnyObject]) -> Void)? = nil
  private var onNavigateToHost: ((String) -> Void)? = nil
  ...
  init(
    payMEFunction: PayMEFunction?, nibName: String?, bundle: Bundle?,
    needHandleNetwork: Bool = false, hasSpinnerHandle: Bool = true
  ) {
    ...
    if form == "" {
      ...
      if myURL != nil {
        myRequest = URLRequest(
          url: myURL!, cachePolicy: .reloadIgnoringLocalAndRemoteCacheData, timeoutInterval: 10)
      } else {
  ...
  func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
    ...
    if wkError.code == NSURLErrorNotConnectedToInternet {
      onError!([
        ...
        "message": wkError.localizedDescription as AnyObject,
      ])
    } else {
      onError!([
  ...
  override func viewDidLoad() {
      ...
      for record in records {
        if record.displayName.contains("payme.com.vn") {
          dataStore.removeData(
            ofTypes: WKWebsiteDataStore.allWebsiteDataTypes(), for: [record],
            completionHandler: {
    ...
    if #available(iOS 9.0, *) {
      ...
      WKWebsiteDataStore.default().removeData(
        ofTypes: websiteDataTypes as! Set<String>, modifiedSince: date as Date,
        completionHandler: {})
    } else {
    ...
    closeButton.trailingAnchor.constraint(equalTo: webView.trailingAnchor, constant: -20).isActive =
      true
  ...
  func userContentController(
      ...
      if let dictionary = message.body as? [String: AnyObject] {
        ...
        if actions == "onRegisterSuccess" {
          if let data = dictionary["data"] as? [String: AnyObject] {
            if let dataInit = data["Init"] as? [String: AnyObject] {
              ...
              payMEFunction?.request.setAccessData(
                kycState == "APPROVED" ? accessToken : "", accessToken, nil)
            }
          }
        ...
        if actions == "onNetWorkError" {
          if let data = dictionary["data"] as? [String: AnyObject] {
            onError!([
              "code": PayME.ResponseCode.NETWORK as AnyObject,
              "message": data["message"] as AnyObject,
            ])
          }
        }
        if actions == "onKYC" {
    ...
    if message.name == onPressScanCard {
      ...
      scanCardController.startScanner(
        onSuccess: { data in
  ...
  public func setOnSuccessCallback(onSuccess: @escaping ([String: AnyObject]) -> Void) {
    self.onSuccess = onSuccess
  }

  public func setOnSuccessWebView(onSuccessWebView: @escaping (String) -> Void) {
    self.onSuccessWebView = onSuccessWebView
  }

  public func setOnFailWebView(onFailWebView: @escaping (String) -> Void) {
    self.onFailWebView = onFailWebView
  }

  public func setOnNavigateToHost(onNavigateToHost: @escaping (String) -> Void) {
    self.onNavigateToHost = onNavigateToHost
  }

  public func setOnErrorCallback(onError: @escaping ([String: AnyObject]) -> Void) {
            ...
            do {
              try store.enumerateContacts(
                ...
                usingBlock: { (contact, stopPointer) in
                  ...
                  let phone =
                    contact.phoneNumbers.first?.value.stringValue.filter("0123456789".contains)
                    ?? ""
                  if let json = try? JSONSerialization.data(
                    withJSONObject: ["name": fullName, "phone": phone] as [String: Any])
                  {
                    if let jsonObj = String(data: json, encoding: String.Encoding.utf8) {
                      contactList += jsonObj + ","
                    }
